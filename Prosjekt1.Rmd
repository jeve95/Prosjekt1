...
```
For the standard error without replacement (`r round(standard_error_with_replacement,3)`), the same model is produced for each sample. That is because when sampling with the same size of the original data set without replacement, each sample naturally are the same. Thus, the standard error is `r standard_error_without_replacement` when bootstrapping without replacement. This indicates no variability in the standard error accros the different bootstrap samples, as all the samples are identical.
