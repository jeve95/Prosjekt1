...
```{r setup2, echo=TRUE, eval=TRUE, cache=TRUE}
data(Boston)
data = Boston
...
```
If the crime is reduced by 10 and age is 60, and then considering all other factor are kept equal our median value of the property is changed by `r valuechanged`.`
...
```
**e)
**i) The gender is a binary qualitative variable. The student has used two binary variables to describe what only needs one. If two binary qualitative variables were used, it would in fact allow for four different cases:
...
I) The decision boundary is given by setting by taking the log of each pdf and adding the log of the prior probability equal each other.
...
```
For the standard error without replacement, the same model is produced for each sample. That is because when sampling with the same size of the original data set without replacement, each samtle naturaly are the same.
